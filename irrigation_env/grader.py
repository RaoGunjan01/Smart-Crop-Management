...
def grade_episode(episode_log: list[dict[str, Any]], *, verbose: bool = True) -> float:
    ...
    water_budget_raw = episode_log[0].get("water_budget", None)
    if water_budget_raw is None:
        water_budget = max(water_used * 2.0, 1.0)
    else:
        water_budget = float(water_budget_raw)
